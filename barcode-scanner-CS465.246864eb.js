...
`;class tF extends HTMLElement{#$=null;#O=null;#H=3e3;static #B=6048e5;static #P(){try{let t=new URLSearchParams(window.location.search),e=t.get("testExpireSeconds"),o=e?Number(e):NaN;if(!Number.isNaN(o)&&o>0)return 1e3*o;let i=t.get("testMode");if("1"===i||"true"===String(i).toLowerCase())return 1e4;if("undefined"!=typeof window)try{let t=window.location.hostname,e=window.location.protocol;if("localhost"===t||"127.0.0.1"===t||"::1"===t||"file:"===e)return 1e4}catch(t){}}catch(t){}return tF.#B}constructor(){super(),this.shadowRoot||this.attachShadow({mode:"open"}).appendChild(tP.content.cloneNode(!0))}async connectedCallback(){this.#$=this.shadowRoot?.getElementById("historyList"),this.#O=this.shadowRoot?.getElementById("emptyHistoryBtn");let[,t=[]]=await tn(),e=(t||[]).map(t=>{if(!t)return null;if("string"==typeof t){let e=Date.now();return{value:t,addedAt:e,expiresAt:e+tF.#P(),notified:!1,preNotified:!1}}return{value:t.value??t?.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+tF.#P():Date.now()+tF.#P()),notified:!!t.notified,preNotified:!!t.preNotified}}).filter(Boolean);await tl(e),this.#F(e||[]);try{let t=new URLSearchParams(window.location.search),e=t.get("testExpireSeconds"),o=t.get("testMode");if(e||o){let t=e?String(Number(e)):"10";try{tA(`Test expiry active: items expire in ${t} second${"1"===t?"":"s"}`,{variant:"warning"})}catch(t){}}}catch(t){}this.#$?.addEventListener("click",this.#j),this.#O?.addEventListener("click",this.#V),this.#U()}disconnectedCallback(){this.#$?.removeEventListener("click",this.#j),this.#O?.removeEventListener("click",this.#V),this.#W()}async add(t){if(!t)return;let e={type:"add",message:"Error adding barcode to history"},[o,i=[]]=await tn();if(o||!Array.isArray(i))return this.#Y("bs-history-error",e),o;let r=(i||[]).map(t=>"string"==typeof t?{value:t,addedAt:Date.now(),expiresAt:Date.now()+tF.#P(),notified:!1,preNotified:!1}:{value:t.value??t.barcode??"",addedAt:t.addedAt??Date.now(),expiresAt:t.expiresAt??(t.addedAt?t.addedAt+tF.#P():Date.now()+tF.#P()),notified:!!t.notified,preNotified:!!t.preNotified}),s="string"==typeof t?t:t.value,a=r.find(t=>t.value===s);if(a){try{let t=tF.#P();if(t<tF.#B){a.expiresAt=Date.now()+t,a.notified=!1,a.preNotified=!1;let[e]=await tl(r);if(!e){let t=this.#$?.querySelector(`li[data-value="${s}"]`);if(t){let e=t.querySelector(".history-countdown");e&&(e.dataset.expiresAt=String(a.expiresAt),e.textContent=this.#G(a.expiresAt-Date.now()))}this.#Y("bs-history-success",{type:"add",message:"Barcode expiry refreshed for testing"}),this.#Z()}return null}}catch(t){}return}let n=Date.now(),l={value:s,addedAt:n,expiresAt:n+tF.#P(),notified:!1,preNotified:!1},d=[...r,l],[c]=await tl(d);return c?(tc.error("Error setting history",c),this.#Y("bs-history-error",e),c):(this.#$?.insertBefore(this.#X(l),this.#$.firstElementChild),this.#Y("bs-history-success",{type:"add",message:"Barcode added to history"}),this.#Z(),null)}async remove(t){if(!t)return;let e={type:"remove",message:"Error removing barcode from history"},[o,i=[]]=await tn();if(o||!Array.isArray(i))return this.#Y("bs-history-error",e),o;let r=(i||[]).filter(e=>"string"==typeof e?e!==t:e.value!==t),[s]=await tl(r);if(s)return tc.error("Error setting history",s),this.#Y("bs-history-error",e),s;let a=this.#$?.querySelector(`li[data-value="${t}"]`);return a?.remove(),this.#Y("bs-history-success",{type:"remove",message:"Barcode removed from history"}),this.#Z(),null}async empty(){let[t]=await tl([]);return t?(tc.error("Error setting history",t),this.#Y("bs-history-error",{type:"empty",message:"Error emptying history"}),t):(this.#$?.replaceChildren(),this.#Y("bs-history-success",{type:"empty",message:"History emptied successfully"}),this.#Z(),null)}#F(t){if(!this.#$)return;this.#$.replaceChildren();let e=document.createDocumentFragment();[...t].reverse().forEach(t=>e.appendChild(this.#X(t))),this.#$.appendChild(e)}#X(t){let e,o=document.createElement("li"),i="string"==typeof t?t:t.value||"",r=t?.expiresAt||Date.now()+tF.#P();o.setAttribute("data-value",i);try{new URL(i),(e=document.createElement("a")).href=i,e.setAttribute("target","_blank"),e.setAttribute("rel","noreferrer noopener")}catch{e=document.createElement("span")}e.textContent=i;let s=document.createElement("span");s.className="history-countdown",s.setAttribute("aria-live","polite"),s.dataset.expiresAt=String(r),s.textContent=this.#G(r-Date.now());let a=document.createElement("div");a.className="actions";let n=document.createElement("custom-clipboard-copy"),l=n.shadowRoot?.querySelector("button");n.setAttribute("only-icon",""),n.setAttribute("value",i),l?.setAttribute("aria-label",`Copy to clipboard ${i}`),a.appendChild(n);let d=document.createElement("button");return d.type="button",d.className="delete-action",d.setAttribute("data-action","delete"),d.setAttribute("aria-label",`Remove from history ${i}`),d.innerHTML=`
    ...
    `},{once:!0}),tk.defineCustomElement();let T=e?.shadowRoot,z=T?.querySelector("video"),R=T?.querySelector('[part="actions-container"]');async function M(){if(A){tc.info("Scanning..."),l?.removeAttribute("hidden");try{let[,t]=await ts(),e=await _.detect(z),o=e?.rawValue??"";if(!o)throw Error("No barcode detected");if(th(s,o),S(o,r),t?.addToHistory)try{await i?.add(o),g&&(g.open=!0,setTimeout(()=>{try{let t=i?.shadowRoot?.querySelector(`li[data-value="${o}"]`);t?.scrollIntoView({behavior:"smooth",block:"center"}),t?.classList?.add("highlight"),setTimeout(()=>t?.classList?.remove("highlight"),2e3)}catch(t){}},50))}catch(t){}if(tb(),!t?.continueScanning){E&&(clearTimeout(E),E=null),d?.removeAttribute("hidden"),u?.setAttribute("hidden",""),R?.setAttribute("hidden","");return}}catch{}A&&(E=setTimeout(()=>M(),1e3))}}async function N(t){if(!t)return;let[,e]=await ts(),o=new Image,r=new FileReader;r.onload=r=>{let s=r.target.result;o.onload=async()=>{try{let t=await _.detect(o),r=t?.rawValue??"";if(!r)throw Error("No barcode detected");if(th(n,r),S(r,a),e?.addToHistory)try{await i?.add(r),g&&(g.open=!0,setTimeout(()=>{try{let t=i?.shadowRoot?.querySelector(`li[data-value="${r}"]`);t?.scrollIntoView({behavior:"smooth",block:"center"}),t?.classList?.add("highlight"),setTimeout(()=>t?.classList?.remove("highlight"),2e3)}catch(t){}},50))}catch(t){}tb()}catch(t){tc.error(t),tA("<strong>No barcode detected</strong><br><small>Please try again with a different image.</small>",{variant:"danger",trustDangerousInnerHTML:!0}),tb({success:!1})}},o.src=s,o.alt="Image preview",c.replaceChildren();let l=document.createElement("div");l.className="dropzone-preview";let d=document.createElement("div");d.className="dropzone-preview__image-wrapper";let h=document.createElement("div");h.className="dropzone-preview__file-name",h.textContent=t.name,d.appendChild(o),l.appendChild(d),l.appendChild(h),c.prepend(l)},r.readAsDataURL(t)}async function q(t){u?.removeAttribute("hidden"),tg(t.detail.video,u),M();let o=t.target.getTrackSettings(),i=t.target.getTrackCapabilities(),r=document.getElementById("zoomLevel");if(i?.torch&&(m?.addEventListener("click",I),m?.removeAttribute("hidden"),e.hasAttribute("torch")&&tE({el:m,isTorchOn:!0})),o?.zoom&&i?.zoom){let t=document.getElementById("zoomControls"),s=i?.zoom?.min||0,a=i?.zoom?.max||10,n=o?.zoom||1;t?.addEventListener("click",t=>{let o=t.target.closest('[data-action="zoom-in"]'),i=t.target.closest('[data-action="zoom-out"]');o&&n<a&&(n+=.5),i&&n>s&&(n-=.5),r.textContent=n.toFixed(1),e.zoom=n}),t?.removeAttribute("hidden"),r.textContent=n.toFixed(1)}let s=await tk.getVideoInputDevices();s.forEach((t,e)=>{let o=document.createElement("option");o.value=t.deviceId,o.textContent=t.label||`Camera ${e+1}`,w.appendChild(o)}),s.length>1&&(w?.addEventListener("change",$),w?.removeAttribute("hidden"))}async function D(t){t.preventDefault();let e={},o=new FormData(y),i=o.getAll("general-settings"),r=o.getAll("formats-settings");i.forEach(t=>e[t]=!0),e.formats=r,ta(e),"formats-settings"===t.target.name&&(_=await tv.create(r))}function I(t){e.torch=!e.torch,tE({el:t.currentTarget,isTorchOn:e.hasAttribute("torch")})}function $(t){if("function"!=typeof e.restartVideoStream)return;let o=t.target.value||void 0;e.restartVideoStream(o)}c.accept=G.join(","),o.supportedFormats=C,d.addEventListener("click",function(){d?.setAttribute("hidden",""),u?.removeAttribute("hidden"),R?.removeAttribute("hidden"),M()}),t.addEventListener("a-tab-show",td(function(t){let e=t.detail.tabId,o=document.querySelector("video-capture");if("cameraTab"===e){if(A=!0,!o)return;if(!o.loading&&d.hasAttribute("hidden")&&(u?.removeAttribute("hidden"),R?.removeAttribute("hidden"),M()),"function"==typeof o.startVideoStream){let t=w?.value||void 0;o.startVideoStream(t)}}else"fileTab"===e&&(A=!1,null!=o&&"function"==typeof o.stopVideoStream&&o.stopVideoStream(),u?.setAttribute("hidden",""),R?.setAttribute("hidden",""))},250)),c.addEventListener("files-dropzone-drop",function(t){N(t.detail.acceptedFiles[0])}),h.addEventListener("resize-observer:resize",function(){tg(e.shadowRoot.querySelector("video"),u)}),f.addEventListener("click",function(){v.open=!0}),y.addEventListener("change",td(D,500)),b.addEventListener("click",function(){g.open=!0}),document.addEventListener("visibilitychange",function(){if("cameraTab"===t.querySelector("[selected]").getAttribute("id"))if("hidden"===document.visibilityState)A=!1,null!=e&&"function"==typeof e.stopVideoStream&&e.stopVideoStream();else{A=!0;let t=document.querySelector("video-capture");if(!t)return;if(!t.loading&&d.hasAttribute("hidden")&&M(),"function"==typeof t.startVideoStream){let e=w?.value||void 0;t.startVideoStream(e)}}}),document.addEventListener("keydown",function(e){"Escape"===e.key&&function(){let e=t.querySelector("#cameraTab").hasAttribute("selected"),o=!d.hidden,i=v.hasAttribute("open"),r=g.hasAttribute("open");o&&e&&!(i||r)&&d.click()}()}),document.addEventListener("bs-history-success",function(t){let{type:e,message:o}=t.detail;"add"===e&&tA(o,{variant:"success"})}),document.addEventListener("bs-history-error",function(t){let{type:e,message:o}=t.detail;("remove"===e||"empty"===e)&&g?.hide(),tA(o,{variant:"danger"})})}();
//# sourceMappingURL=barcode-scanner-CS465.246864eb.js.map
